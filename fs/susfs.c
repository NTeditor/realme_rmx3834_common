...
#define SUSFS_LOGI(fmt, ...)                                                   \
	...
	pr_info("susfs:[%u][%d][%s] " fmt, current_uid().val, current->pid,    \
		__func__, ##__VA_ARGS__)
#define SUSFS_LOGE(fmt, ...)                                                   \
	if (susfs_is_log_enabled)                                              \
	pr_err("susfs:[%u][%d][%s]" fmt, current_uid().val, current->pid,      \
	       __func__, ##__VA_ARGS__)
#else
#define SUSFS_LOGI(fmt, ...)
#define SUSFS_LOGE(fmt, ...)
...
#endif
const struct qstr susfs_fake_qstr_name = QSTR_INIT(
	"..5.u.S",
	7); // used to re-test the dcache lookup, make sure you don't have file named like this!!
...
out_copy_to_user:
	...
	if (info.cmd == CMD_SUSFS_SET_ANDROID_DATA_ROOT_PATH) {
		SUSFS_LOGI(
			"CMD_SUSFS_SET_ANDROID_DATA_ROOT_PATH deprecated, will be removed soon, ret: %d\n",
			info.err);
	} else if (info.cmd == CMD_SUSFS_SET_SDCARD_ROOT_PATH) {
		SUSFS_LOGI(
			"CMD_SUSFS_SET_SDCARD_ROOT_PATH, deprecated, will be removed soon, ret: %d\n",
			info.err);
	}
}
...
void susfs_add_sus_path(void __user **user_info)
{
	...
	if (inode->i_sb->s_magic == FUSE_SUPER_MAGIC) {
		...
		SUSFS_LOGI(
			"flagged AS_FLAGS_SUS_PATH on pathname: '%s', fi->nodeid: %llu, fi->inode.i_ino: %lu, fi->inode.i_mapping->flags: 0x%lx\n",
			info.target_pathname, fi->nodeid, fi->inode.i_ino,
			fi->inode.i_mapping->flags);
		info.err = 0;
		goto out_path_put_path;
	}
	...
	SUSFS_LOGI(
		"flagged AS_FLAGS_SUS_PATH on pathname: '%s', ino: '%lu', inode->i_mapping->flags: 0x%lx\n",
...
void susfs_add_sus_path_loop(void __user **user_info)
{
	...
	strncpy(new_list->info.target_pathname, info.target_pathname,
		SUSFS_MAX_LEN_PATHNAME - 1);
	strncpy(new_list->target_pathname, info.target_pathname,
		SUSFS_MAX_LEN_PATHNAME - 1);
	...
	SUSFS_LOGI(
		"target_ino: '%lu', target_pathname: '%s', i_uid: '%u', is successfully added to LH_SUS_PATH_LOOP\n",
...
void susfs_run_sus_path_loop(uid_t uid)
{
	...
	list_for_each_entry_rcu (cursor, &LH_SUS_PATH_LOOP, list) {
		if (!kern_path(cursor->target_pathname, 0, &path)) {
			...
			path_put(&path);
			SUSFS_LOGI(
				"re-flag AS_FLAGS_SUS_PATH on path '%s' for uid: %u\n",
...
bool susfs_is_inode_sus_path(struct mnt_idmap *idmap, struct inode *inode)
{
	...
	if (inode->i_sb->s_magic == FUSE_SUPER_MAGIC) {
		...
		if (unlikely(
			    test_bit(AS_FLAGS_SUS_PATH,
				     &fi->inode.i_mapping->flags) &&
			    is_i_uid_not_allowed(
				    i_uid_into_vfsuid(idmap, &fi->inode).val))) {
	...
	if (unlikely(test_bit(AS_FLAGS_SUS_PATH, &inode->i_mapping->flags) &&
		     is_i_uid_not_allowed(
			     i_uid_into_vfsuid(idmap, inode).val))) {
		SUSFS_LOGI("hiding path with ino '%lu'\n", inode->i_ino);
		return true;
	}
	return false;
...
bool susfs_is_inode_sus_path(struct inode *inode)
{
	...
	if (inode->i_sb->s_magic == FUSE_SUPER_MAGIC) {
		...
		if (unlikely(test_bit(AS_FLAGS_SUS_PATH,
				      &fi->inode.i_mapping->flags) &&
			     is_i_uid_not_allowed(
				     i_uid_into_mnt(i_user_ns(&fi->inode),
						    &fi->inode)
					     .val))) {
			SUSFS_LOGI("hiding path with ino '%lu'\n",
				   inode->i_ino);
...
void susfs_set_hide_sus_mnts_for_non_su_procs(void __user **user_info)
{
	...
	info.err = 0;
out_copy_to_user:
	if (copy_to_user(&((struct st_susfs_hide_sus_mnts_for_non_su_procs
				    __user *)*user_info)
				  ->err,
			 &info.err, sizeof(info.err))) {
		info.err = -EFAULT;
...
void susfs_add_sus_kstat(void __user **user_info)
{
	...
	new_entry =
		kmalloc(sizeof(struct st_susfs_sus_kstat_hlist), GFP_KERNEL);
	if (!new_entry) {
		info.err = -ENOMEM;
		goto out_copy_to_user;
	}
	...
	info.err =
		susfs_update_sus_kstat_inode(new_entry->info.target_pathname);
	if (info.err) {
		kfree(new_entry);
		goto out_copy_to_user;
	}
	...
	SUSFS_LOGI(
		"is_statically: '%d', target_ino: '%lu', target_pathname: '%s', spoofed_ino: '%lu', spoofed_dev: '%lu', spoofed_nlink: '%u', spoofed_size: '%llu', spoofed_atime_tv_sec: '%ld', spoofed_mtime_tv_sec: '%ld', spoofed_ctime_tv_sec: '%ld', spoofed_atime_tv_nsec: '%ld', spoofed_mtime_tv_nsec: '%ld', spoofed_ctime_tv_nsec: '%ld', spoofed_blksize: '%lu', spoofed_blocks: '%llu', is successfully added to SUS_KSTAT_HLIST\n",
		new_entry->info.is_statically, new_entry->info.target_ino,
		new_entry->info.target_pathname, new_entry->info.spoofed_ino,
		new_entry->info.spoofed_dev, new_entry->info.spoofed_nlink,
		new_entry->info.spoofed_size,
		new_entry->info.spoofed_atime_tv_sec,
		new_entry->info.spoofed_mtime_tv_sec,
		new_entry->info.spoofed_ctime_tv_sec,
		new_entry->info.spoofed_atime_tv_nsec,
		new_entry->info.spoofed_mtime_tv_nsec,
		new_entry->info.spoofed_ctime_tv_nsec,
		new_entry->info.spoofed_blksize,
		new_entry->info.spoofed_blocks);
#else
	SUSFS_LOGI(
		"is_statically: '%d', target_ino: '%lu', target_pathname: '%s', spoofed_ino: '%lu', spoofed_dev: '%lu', spoofed_nlink: '%u', spoofed_size: '%u', spoofed_atime_tv_sec: '%ld', spoofed_mtime_tv_sec: '%ld', spoofed_ctime_tv_sec: '%ld', spoofed_atime_tv_nsec: '%ld', spoofed_mtime_tv_nsec: '%ld', spoofed_ctime_tv_nsec: '%ld', spoofed_blksize: '%lu', spoofed_blocks: '%llu', is successfully added to SUS_KSTAT_HLIST\n",
		new_entry->info.is_statically, new_entry->info.target_ino,
		new_entry->info.target_pathname, new_entry->info.spoofed_ino,
		new_entry->info.spoofed_dev, new_entry->info.spoofed_nlink,
		new_entry->info.spoofed_size,
...
void susfs_update_sus_kstat(void __user **user_info)
{
	...
	hash_for_each_safe (SUS_KSTAT_HLIST, bkt, tmp_node, tmp_entry, node) {
		if (!strcmp(tmp_entry->info.target_pathname,
			    info.target_pathname)) {
			info.err = susfs_update_sus_kstat_inode(
				tmp_entry->info.target_pathname);
			if (info.err) {
				goto out_copy_to_user;
			}
			new_entry =
				kmalloc(sizeof(struct st_susfs_sus_kstat_hlist),
					GFP_KERNEL);
			if (!new_entry) {
				info.err = -ENOMEM;
				goto out_copy_to_user;
			...
			SUSFS_LOGI(
				"updating target_ino from '%lu' to '%lu' for pathname: '%s' in SUS_KSTAT_HLIST\n",
			...
			if (info.spoofed_size > 0) {
				SUSFS_LOGI(
					"updating spoofed_size from '%lld' to '%lld' for pathname: '%s' in SUS_KSTAT_HLIST\n",
					new_entry->info.spoofed_size,
					info.spoofed_size,
					info.target_pathname);
				new_entry->info.spoofed_size =
			...
			if (info.spoofed_blocks > 0) {
				SUSFS_LOGI(
					"updating spoofed_blocks from '%llu' to '%llu' for pathname: '%s' in SUS_KSTAT_HLIST\n",
					new_entry->info.spoofed_blocks,
					info.spoofed_blocks,
					info.target_pathname);
				new_entry->info.spoofed_blocks =
			...
			kfree(tmp_entry);
			spin_lock(&susfs_spin_lock_sus_kstat);
			hash_add(SUS_KSTAT_HLIST, &new_entry->node,
				 info.target_ino);
			spin_unlock(&susfs_spin_lock_sus_kstat);
			info.err = 0;
			goto out_copy_to_user;
...
void susfs_sus_ino_for_show_map_vma(unsigned long ino, dev_t *out_dev,
...
void susfs_spoof_uname(struct new_utsname *tmp)
{
	if (unlikely(my_uname.release[0] == '\0' ||
		     spin_is_locked(&susfs_spin_lock_set_uname)))
		return;
...
void susfs_set_cmdline_or_bootconfig(void __user **user_info)
{
	struct st_susfs_spoof_cmdline_or_bootconfig *info =
		(struct st_susfs_spoof_cmdline_or_bootconfig *)kzalloc(
	...
	if (copy_from_user(
		    info,
		    (struct st_susfs_spoof_cmdline_or_bootconfig __user
			     *)*user_info,
		    sizeof(struct st_susfs_spoof_cmdline_or_bootconfig))) {
		info->err = -EFAULT;
		goto out_copy_to_user;
	...
	if (!fake_cmdline_or_bootconfig) {
		fake_cmdline_or_bootconfig = (char *)kzalloc(
	...
	strncpy(fake_cmdline_or_bootconfig, info->fake_cmdline_or_bootconfig,
...
out_copy_to_user:
	...
	if (copy_to_user(&((struct st_susfs_spoof_cmdline_or_bootconfig __user
				    *)*user_info)
				  ->err,
			 &info->err, sizeof(info->err))) {
		info->err = -EFAULT;
...
static int
susfs_update_open_redirect_inode(struct st_susfs_open_redirect_hlist *new_entry)
{
	struct path path;
	struct inode *inode;
	int err = 0;
...
void susfs_add_open_redirect(void __user **user_info)
{
	...
	new_entry = kmalloc(sizeof(struct st_susfs_open_redirect_hlist),
			    GFP_KERNEL);
	if (!new_entry) {
		info.err = -ENOMEM;
		goto out_copy_to_user;
	...
	new_entry->target_ino = info.target_ino;
	strncpy(new_entry->target_pathname, info.target_pathname,
		SUSFS_MAX_LEN_PATHNAME - 1);
	strncpy(new_entry->redirected_pathname, info.redirected_pathname,
		SUSFS_MAX_LEN_PATHNAME - 1);
	...
	SUSFS_LOGI(
		"target_ino: '%lu', target_pathname: '%s' redirected_pathname: '%s', is successfully added to OPEN_REDIRECT_HLIST\n",
		new_entry->target_ino, new_entry->target_pathname,
		new_entry->redirected_pathname);
	info.err = 0;
out_copy_to_user:
	if (copy_to_user(
...
void susfs_set_avc_log_spoofing(void __user **user_info)
{
	...
	info.err = 0;
out_copy_to_user:
	if (copy_to_user(&((struct st_susfs_avc_log_spoofing __user *)*user_info)
				  ->err,
			 &info.err, sizeof(info.err))) {
		info.err = -EFAULT;
	}
...
void susfs_get_enabled_features(void __user **user_info)
{
	struct st_susfs_enabled_features *info =
		(struct st_susfs_enabled_features *)kzalloc(
			sizeof(struct st_susfs_enabled_features), GFP_KERNEL);
	char *buf_ptr = NULL;
	...
	if (copy_from_user(
		    info, (struct st_susfs_enabled_features __user *)*user_info,
	...
	info->err =
		copy_config_to_buf("CONFIG_KSU_SUSFS_SUS_PATH\n", buf_ptr,
				   &copied_size, SUSFS_ENABLED_FEATURES_SIZE);
	if (info->err)
		goto out_copy_to_user;
...
#ifdef CONFIG_KSU_SUSFS_SUS_KSTAT
	info->err =
		copy_config_to_buf("CONFIG_KSU_SUSFS_SUS_KSTAT\n", buf_ptr,
				   &copied_size, SUSFS_ENABLED_FEATURES_SIZE);
	if (info->err)
		goto out_copy_to_user;
	buf_ptr = info->enabled_features + copied_size;
...
#ifdef CONFIG_KSU_SUSFS_SPOOF_UNAME
	info->err =
		copy_config_to_buf("CONFIG_KSU_SUSFS_SPOOF_UNAME\n", buf_ptr,
				   &copied_size, SUSFS_ENABLED_FEATURES_SIZE);
	if (info->err)
		goto out_copy_to_user;
	buf_ptr = info->enabled_features + copied_size;
...
#ifdef CONFIG_KSU_SUSFS_ENABLE_LOG
	info->err =
		copy_config_to_buf("CONFIG_KSU_SUSFS_ENABLE_LOG\n", buf_ptr,
				   &copied_size, SUSFS_ENABLED_FEATURES_SIZE);
	if (info->err)
		goto out_copy_to_user;
	buf_ptr = info->enabled_features + copied_size;
	...
	info->err =
		copy_config_to_buf("CONFIG_KSU_SUSFS_HIDE_KSU_SUSFS_SYMBOLS\n",
				   buf_ptr, &copied_size,
...
#ifdef CONFIG_KSU_SUSFS_OPEN_REDIRECT
	info->err =
		copy_config_to_buf("CONFIG_KSU_SUSFS_OPEN_REDIRECT\n", buf_ptr,
				   &copied_size, SUSFS_ENABLED_FEATURES_SIZE);
	if (info->err)
		goto out_copy_to_user;
	buf_ptr = info->enabled_features + copied_size;
...
#ifdef CONFIG_KSU_SUSFS_SUS_MAP
	info->err =
		copy_config_to_buf("CONFIG_KSU_SUSFS_SUS_MAP\n", buf_ptr,
				   &copied_size, SUSFS_ENABLED_FEATURES_SIZE);
	if (info->err)
		goto out_copy_to_user;
	buf_ptr = info->enabled_features + copied_size;
...
void susfs_show_variant(void __user **user_info)
{
	...
	strncpy(info.susfs_variant, SUSFS_VARIANT,
...
void susfs_show_version(void __user **user_info)
{
	...
	strncpy(info.susfs_version, SUSFS_VERSION,
...
static struct watch_dir g_watch = {
	.path = "/data/media/0", // we choose the underlying f2fs /data/media/0 instead of the FUSE /sdcard
...
void susfs_start_sdcard_monitor_fn(void)
{
	if (IS_ERR(kthread_run(susfs_sdcard_monitor_fn, NULL,
		...
		SUSFS_LOGI(
			"set susfs_is_sdcard_android_data_decrypted to true\n");
