...
#endif
#ifdef CONFIG_KSU_SUSFS_SUS_KSTAT
		if (unlikely(test_bit(AS_FLAGS_SUS_KSTAT,
				      &inode->i_mapping->flags))) {
			      ...
			      struct mem_size_stats *mss)
{
	...
	seq_put_decimal_ull_width(m, " kB\nUss_Filecache_All:\t",
				  (unsigned long)((priv->filecache_uss +
						   mss->private_filecache) >>
						  10),
				  8);
	seq_put_decimal_ull_width(m, " kB\nUss_Anonymous_All:\t",
				  (unsigned long)((priv->anonymous_uss +
						   mss->private_anonymous) >>
						  10),
				  8);
	seq_put_decimal_ull_width(
		m, " kB\nSwap_All:\t",
	...
	seq_put_decimal_ull_width(
		m, " kB\nSwap_Pss_All:\t",
		(unsigned long)((priv->swap_pss + mss->swap_pss) >>
				(10 + PSS_SHIFT)),
		8);
	seq_put_decimal_ull_width(
		m, " kB\nSwap_Uss_All:\t",
		(unsigned long)((priv->swap_uss + mss->swap_uss) >>
				(10 + PSS_SHIFT)),
		8);
	seq_puts(m, " kB\n");
}
...
static void smaps_page_accumulate(struct mem_size_stats *mss, struct page *page,
				  unsigned long size, unsigned long pss,
...
#ifdef CONFIG_ENHANCE_SMAPS_INFO
	SEQ_PUT_DEC(" kB\nSwapUss:        ", mss->swap_uss >> PSS_SHIFT);
#endif
	...
	if (type == RECLAIM_RANGE) {
		...
		while (vma) {
			...
			walk_page_range(mm, max(vma->vm_start, start),
					min(vma->vm_end, end), &ppr_walk_ops,
					&rp);
			vma = vma->vm_next;
		}
	} else {
